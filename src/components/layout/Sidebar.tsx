...
export default function Sidebar({ guildId, onNavigate }: { guildId: string; onNavigate?: () => void }) {
  ...
  return (
    <div className="glass-card flex h-full w-full flex-col rounded-none border-y-0 border-l-0 bg-[linear-gradient(180deg,_rgba(10,19,30,0.95)_0%,_rgba(9,16,25,0.95)_100%)]">
      ...
      <nav className="flex-1 space-y-6 overflow-y-auto px-3 py-4">
        {navGroups.map((group) => (
          <div key={group.label}>
            <p className="mb-2 px-2 text-[10px] font-bold uppercase tracking-[0.18em] text-discord-text-muted/85">
              {group.label}
            </p>
